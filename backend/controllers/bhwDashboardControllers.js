...
exports.getBhwStats = async (req, res) => {
  ...
  try {
    const [[childStats]] = await pool.query(
      `SELECT COUNT(*) AS totalChildren FROM children WHERE barangay = ? AND status = 'active'`,
      [barangay]
    );

    const [[motherStats]] = await pool.query(
      `SELECT COUNT(*) AS totalMothers FROM mothers WHERE barangay = ? AND status = 'active'`,
      [barangay]
    );

    // Latest nutrition record per child in this barangay
    const [latestRecords] = await pool.query(
      `SELECT nr.weight_status, nr.height_status, nr.wasting_status, nr.overall_status
    ...
    latestRecords.forEach((r) => {
      ...
      if (r.wasting_status === 'wasted' || r.wasting_status === 'severely_wasted') wasted++;
    ...
    const [[atRiskMothersRow]] = await pool.query(
      `SELECT COUNT(*) AS atRiskMothers FROM mothers WHERE barangay = ? AND status = 'active' AND weight_kg IS NOT NULL AND weight_kg < 45`,
      [barangay]
    );
