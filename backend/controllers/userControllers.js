  ...
  try {
    ...
    exports.getUserStats = async (req, res) =>{
        try{
            const [[stats]] = await pool.query(
                `SELECT
                SUM(CASE WHEN role = 'bhw' THEN 1 ELSE 0 END) AS totalBHW,
                SUM(CASE WHEN role = 'bns' THEN 1 ELSE 0 END) AS totalBNS,
                SUM(CASE WHEN role = 'bhw' AND status = 'active' THEN 1 ELSE 0 END) AS activeBHW,
                SUM(CASE WHEN role = 'bns' AND status = 'active' THEN 1 ELSE 0 END) AS activeBNS
                FROM users
    ...
    exports.updateUser = async (req, res) => {
        ...
        try{
            ...
            await pool.query(
                'UPDATE users SET first_name = ?, middle_initial = ?, last_name = ?, email = ?, username = ?, password = ?, role = ?, municipality = ?, barangay = ? WHERE user_id = ?',
                [first_name, middle_initial || null, last_name, email, username, password, role, municipality, barangay, user_id]
            );
