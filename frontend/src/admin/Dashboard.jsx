...
function StatCard({ icon: Icon, label, value, sublabel, iconBg }) {
    return (
        <div className="motion-card bg-white rounded-2xl shadow-sm p-5 relative hover:shadow-md transition-shadow">
            <div className="flex items-start justify-between">
                <p className="text-sm text-gray-400">{label}</p>
                <div className={`${iconBg || 'bg-green-600'} rounded-full p-2.5`}>
...
function Dashboard() {
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-gradient-to-r from-green-600 to-emerald-500 rounded-2xl p-6 flex items-center justify-between text-white shadow-sm">
                <div>
                    <h2 className="text-xl font-bold">Hello, {user.full_name || 'Admin'}!</h2>
                    <p className="text-sm text-white/80 mt-1">All municipal nutrition status overview</p>
                </div>
                <div className="flex items-center gap-2 bg-white/20 backdrop-blur-sm text-sm px-4 py-2 rounded-full">
                    <Bell size={14} />
                    <span className="break-words leading-tight">
                        {stats.upcomingActivities?.length > 0
                            ? `${stats.upcomingActivities.length} Upcoming Activities`
                    ...
                    <div className="space-y-3">
                        {Object.entries(stats.nineCategoryTrend).map(([key, count]) => (
                            <div key={key} className="flex items-center gap-3">
                                <span className="text-xs text-gray-500 w-28 shrink-0 break-words leading-tight">{statusLabels[key]}</span>
                            ...
                            stats.upcomingActivities.map((activity) => {
                                ...
                                return (
                                    <div key={activity.schedule_id} className="flex items-center gap-3 p-3 rounded-xl bg-green-50/45 border border-green-100/70 hover:bg-green-50 transition">
                                        <div className="w-11 h-11 rounded-xl bg-green-50 flex flex-col items-center justify-center text-green-700 flex-shrink-0">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <p className="text-sm font-medium text-gray-800 break-words leading-snug">{activity.title}</p>
