...
function UserManagement() {
  ...
  return (
    <div>
      ...
      {/* Stat cards — clickable, nagfi-filter agad */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <StatCard
          label="Total BHW"
          value={stats.totalBHW}
        ...
        <StatCard
          label="Active (BHW)"
          value={stats.activeBHW}
          sublabel={`${barangays.length} barangays`}
          onClick={() => { setActiveTab('bhw'); setStatusFilter('active'); }}
          isActive={activeTab === 'bhw' && statusFilter === 'active'}
        />
        <StatCard
          label="Active (BNS)"
          value={stats.activeBNS}
          sublabel={`${barangays.length} barangays`}
          onClick={() => { setActiveTab('bns'); setStatusFilter('active'); }}
          isActive={activeTab === 'bns' && statusFilter === 'active'}
        />
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm p-4 mb-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex gap-2">
            ...
            {/* ✅ Iisang "Add User" button na lang, role kinukuha mula sa activeTab */}
            <button
              onClick={openAddModal}
              className="flex items-center gap-2 bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-green-700 text-sm"
            >
              <Plus size={16} /> Add {activeTab === 'bhw' ? 'BHW' : 'BNS'}
            </button>
          </div>
        </div>
          ...
          <div className="bg-white rounded-xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <input name="barangay" placeholder="Barangay" value={form.barangay} onChange={handleChange} disabled={viewOnly} required className="border rounded-lg px-3 py-2 text-sm disabled:bg-gray-50" />
              </div>
              <div className="grid grid-cols-2 gap-3">
